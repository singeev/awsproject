...
public class SignupController {
    ...
    public String signUpPost(@RequestParam(name = "planId", required = true) int planId,
        ...
        if (model.containsKey(DUPLICATED_USERNAME_KEY)) {
            LOGGER.warn("The username already exists. Displaying error to the user");
            model.addAttribute(SIGNED_UP_MESSAGE_KEY, "false");
        ...
        if (model.containsKey(DUPLICATED_EMAIL_KEY)) {
            LOGGER.warn("The email already exists. Displaying error to the user");
            model.addAttribute(SIGNED_UP_MESSAGE_KEY, "false");
...
//            } else {
//                LOGGER.warn("There was a problem uploading the profile image to S3. The user's profile will" +
...
//            LOGGER.info("Subscribing the customer to plan {}", selectedPlan.getName());
